...
数据落点：`<cwd>/.deep-intent-collaboration/explore-tree/tree.json`（惰性创建，首次 `add` 时生成）。
...
调用：`node <skill-dir>/scripts/explore-tree.js <cmd> [args]`（skill-dir = SKILL.md 所在目录）。

### 维护类

```
init                          建空探索树（一般无需手动；首次 add 自动 init + migrate）
...
done <id> [--choice "X"]      标记叶节点已解决（可记所选选项）
                              自动重算受影响节点状态 + 推进 current
                              有子节点的节点不能直接 done（先解决子节点）

abandon <id> [--reason "..."] 标记死路（含"压根不可行"和"试过不通"，均留痕降权）
                              非叶子 → 整子树标记 abandoned

note <id> <text>              给节点附备注/结论/放弃理由（可多次追加）
...
status                        一行摘要：根数 / 节点数 / 各状态计数 + 当前问题

focus <id> | focus --clear    手动钉住 / 清空 current
...
```
[分叉点出现]
  → node explore-tree.js add "技术选型" --parent <当前问题id> --type 分叉 --options "Redis|RabbitMQ|原生队列"
  → 把选择抛给用户

[用户选完]
  → node explore-tree.js done <该节点id> --choice "Redis"
  → 工具自动重算 + 推进 current

[用户要求下钻某个细节]
  → node explore-tree.js add "Redis 集群方案" --parent <该节点id> --type 下钻

[探索后发现某方案不可行]
  → node explore-tree.js abandon <节点id> --reason "服务商不支持"

[改名 / 批量回填备注 / 记录已选选项]
  → node explore-tree.js update '{"n3":{"question":"更准的问题"}, "n5":{"resolved_choice":"Redis","note":"选 Redis 因 X"}}'
  → 或 --file patch.json（内容大 / 含换行时）

[想看现在走到哪了]
  → node explore-tree.js status     （一行摘要）
  → node explore-tree.js show       （聚焦视图，折叠远的）

[复盘 / 看全局]
  → node explore-tree.js show all
```
...
- 无多树归档机制（会话树默认覆盖；需保留历史时，阶段三手动把关键路径 `show all` 的输出归档进 `experience-log/`）。
